 ...
 * $maxnumber determines how many videos are displayed on a page.
 */
function ptogo_process_response($id, $instance, $format = 'list', $maxnumber = 20) {
    global $DB;
    ...
    // Display an error when we can't reach the server.
    if (!$response) {
        \core\notification::error(get_string('serverdown', 'ptogo'));
        return false;
    }
    // Display if we received errors from server.
    if(isset($response->error) && $response->error !== "") {
        \core\notification::error("Error: " . $response->error);
        return false;
    }
    ...
    foreach($response->response as $video) {
        ...
        if ($ptogoinstance->video_id == null || in_array($video->id, $items)) {
            ...
            } else if ($format == 'embed' && $maxnumber > 0) {
                //$html .=  "<embed width='500px' src='" . $video->embedUrl . "'>"; // According to P2G embed does not work at all.
            ...
            $maxnumber--; // Decrement to keep track of videos shown.
        }
    }
    return $html;
}
