function queryWizard(responseTo, submitString, plugin_url) {
    ...
    this.getFilters = function() {
        ...
        xmlHttp.onload = function() {
            ...
            if(xmlHttp.responseText.startsWith('{"schemaVersion"')) {
                response = JSON.parse(xmlHttp.responseText);
                return this.show();
            } else {
                alert("Sorry, we can't reach the server."); // TODO: Multilang, maybe prettify.
            }
        }.bind(this);
    };
        ...
        for(i=0;i<results.length;i++) {
            if(results[i].value == value) {
                ...
                if(results[i].useVocabularies === "true") {
                    ...
                    // If it is a select field remove select value0 and replace with empty text.
