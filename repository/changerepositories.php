...
class mod_ptogo_repository_change extends moodleform {
    ...
    function validation($data, $files) {
        ...
        if($data) {
            ...
            if ($data['expiration'] < 1) {
                $errors['expiration'] = get_string('biggerthanzero', 'ptogo');
            }
            ...
            if(!$this->accessptogo($data['serverurl'])) {
                $errors['serverurl'] = get_string('serverurl_error', 'ptogo');
            }
        }
        return $errors;
    }
    ...
    /*
     * Small function to determine if a server can actually be reached before saving.
     */
    function accessptogo($serverurl, $secretkey = '') {
        $timeout = 10;
        $ch = curl_init();
