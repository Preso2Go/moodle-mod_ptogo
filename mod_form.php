...
class mod_ptogo_mod_form extends moodleform_mod {
    function __construct($data, $section, $cm, $course)
    {
        ...
        // TODO: Consolidate the requires and put them in amd.
    ...
    function definition() {
        ...
        // Show embedded videos on course page.
        $mform->addElement('advcheckbox', 'showinlisting', 'Display', get_string('showinlisting', 'ptogo'));
        $mform->addHelpButton('showinlisting', 'showinlisting', 'ptogo');
        ...
        // Fails if no repos are set up. Redirect and write error message.
        if(empty($repositories)) {
            Global $COURSE;
            $url = new moodle_url('/course/view.php', array('id' => $COURSE->id ));
            $message = get_string('norepos', 'ptogo');
        ...
        $aquery = $mform->addElement('hidden', 'baseQuery', ' '); // TODO: This has a false name, should be additional_query all the way.
        ...
        function changeRepositoryData(data) {
            ...
            // Set values in moodle form for storing.
            ...
            new itemPreview("id_ptogo_item_id", "' . get_string("item_add", "ptogo") . '", "' . new moodle_url("/mod/ptogo/") . '", true, "", "' . get_string("item_add", "ptogo") . '");
        }

        // Here the searchicon gets its functionality.
        function attachNewEvent() {
            document.getElementById("search").addEventListener("click", function() {
                var preview = new itemPreview("id_ptogo_item_id", "' . get_string("item_add", "ptogo") . '", "' . new moodle_url("/mod/ptogo/") . '", true, "", "' . get_string("item_add", "ptogo") . '");
            });
        }

        // Build queryWizard first.
        ...
        if(isset($this->current->update) && is_int($this->current->update)) {
            ...
            $mform->addElement('html', '<script>
                window.onload = function() {
                    var preview =  new itemPreview("id_ptogo_item_id", "' . get_string("item_add", "ptogo") . '", "' . new moodle_url("/mod/ptogo/") . '", true, ' . $jsdata . ', "' . get_string("item_add", "ptogo") . '");
